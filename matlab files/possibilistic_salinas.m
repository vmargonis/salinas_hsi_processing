...
% min-max init for theta
[~, theta_init] = most_dist_repre(Y, m);
theta_init = theta_init';

% initialize eta via fuzzy-c-means:
eta = zeros(1,m);
[~,N]=size(Y);
max_iter = 1000;
[theta_fcm, U_fcm] = fuzzy(Y, m, theta_init, q, max_iter);

U_fcm = U_fcm .^ q; %Nxm
for j=1:m
	d = sum( (Y - theta_fcm(j,:)'*ones(1,N)).^2 )' ; %Nx1
	numer = sum(U_fcm(:,j) .* d);
	denom = sum(U_fcm(:,j));
	eta(j) = numer / denom;
end
...
tic;
% GPAS1
[theta, U] = possibilistic_1(Y, m, theta_init, eta, q, max_iter);

% GPAS 2
%[theta, U] = possibilistic_2(Y, m, theta_init, eta, max_iter);
run_time=toc;
[~, bel] = max(U, [], 2);
